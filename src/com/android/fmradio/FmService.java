...
public class FmService extends Service implements FmRecorder.OnRecorderStateChangedListener {
    ...
    private FMMediaButtonIntentReceiver mMediaButtonIntentReceiver = new FMMediaButtonIntentReceiver();
    private FMMediaButtonIntentReceiver.MediaHandleListener listener = new FMMediaButtonIntentReceiver.MediaHandleListener() {
        @Override
        public boolean handleIntent(Intent intent) {
            ...
            switch (keyCode) {
                ...
                case KeyEvent.KEYCODE_MEDIA_STEP_FORWARD:
                    if (repeatCount==0) {
                        tuneStationAsync(keyCode == KeyEvent.KEYCODE_MEDIA_SKIP_BACKWARD || keyCode == KeyEvent.KEYCODE_MEDIA_STEP_BACKWARD? FmUtils.computeDecreaseStation(mCurrentStation):FmUtils.computeIncreaseStation(mCurrentStation));
                        result = true;
                        break;
                    }
                    else {
                        keyCode = keyCode == KeyEvent.KEYCODE_MEDIA_SKIP_BACKWARD || keyCode == KeyEvent.KEYCODE_MEDIA_STEP_BACKWARD ? KeyEvent.KEYCODE_MEDIA_PREVIOUS : KeyEvent.KEYCODE_MEDIA_NEXT;
                        result = true;
                    }
                case KeyEvent.KEYCODE_MEDIA_PREVIOUS:
                case KeyEvent.KEYCODE_MEDIA_NEXT:
                    if (repeatCount==0) {
                        seekStationAsync(FmUtils.computeFrequency(mCurrentStation), keyCode==KeyEvent.KEYCODE_MEDIA_NEXT);
                        result = true;
                    }
                    break;
                default:
    ...
    public void onCreate() {
        ...
        Intent i=new Intent(this,FMMediaButtonIntentReceiver.class).setFlags(Intent.FLAG_RECEIVER_FOREGROUND);
        ...
        mMediaSession.setPlaybackState(new PlaybackState.Builder().setActions(PlaybackState.ACTION_PAUSE
